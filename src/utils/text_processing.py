...
def extract_clean_response(text: str) -> Dict[str, Any]:
    ...
    # 修复常见格式问题
    cleaned_text = _fix_json_format(cleaned_text)
    ...
    array_pattern = r'\[[\s\S]*?\]'
    match = re.search(array_pattern, cleaned_text)
    if match:
        try:
            return json.loads(match.group())
        except JSONDecodeError:
    ...
    return {"error": "JSON解析失败", "raw_text": cleaned_text, "cleaned_text": cleaned_text}
